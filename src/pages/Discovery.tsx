...
export default function Discovery() {
  ...
  // Rechercher les streamers Twitch
  const searchTwitchStreamers = async (searchTermOverride?: string, languageOverride?: string) => {
    ...
    try {
      ...
      if (data && data.streams) {
        ...
        toast({
          ...
          description: `${data.streams.length} stream(s) trouvé(s) pour "${termToSearch}"${languageToSearch !== 'all' ? ` en ${LANGUAGES.find(l => l.code === languageToSearch)?.name}` : ''}`,
        });
      }
    } catch (error) {
        ...
        <Tabs defaultValue="pauvrathon" className="w-full mb-8">
          ...
          <TabsContent value="twitch">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="text-sm text-muted-foreground bg-muted/50 p-3 rounded-lg">
                ...
                {selectedLanguage !== 'all' && (
                  <span> en <span className="font-medium">{LANGUAGES.find(l => l.code === selectedLanguage)?.name}</span></span>
                )}
              </div>
            </div>
            ...
            {loadingTwitch ? (
              ...
              <Card className="text-center py-8">
                <CardContent>
                  ...
                  <p className="text-muted-foreground">
                    Aucun streamer avec "{searchTerm}" dans le titre n'a été trouvé
            ...
            <div className="mt-6 text-center text-muted-foreground text-sm">
              <p>
                Ces streams sont en direct sur Twitch avec "{searchTerm}" dans leur titre. 
                Ils ne sont pas nécessairement affiliés à notre plateforme.
              </p>
              <p className="mt-1">
                Résultats: {twitchStreamers.length} stream(s) - Langue: {LANGUAGES.find(l => l.code === selectedLanguage)?.name}
              </p>
            </div>
          </TabsContent>
        </Tabs>
